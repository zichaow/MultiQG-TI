...
parser.add_argument("--do_sample", type=lambda x: (str(x).lower() == 'true'), default=True, help="Do sample")
...
def generate(url, prompt, model, processor, gen_mode='C',
             ...
             top_p=0.95, min_length=0, n_samples=10, max_length=100, half_prec=True):
    outputs = []
    ppls = []
    # image = Image.open(requests.get(url, stream=True).raw).convert('RGB')
    image = Image.open(url).convert('RGB')
    datatype = torch.float16 if half_prec else torch.float32
    inputs = processor(images=image, text=prompt, return_tensors="pt").to(device, datatype)
    ...
    for idx in range(generation['sequences'].shape[0]):
        ...
        # set_trace()
        # new_inputs = processor(images=image, text=gen_text, return_tensors="pt").to(device, datatype)
        # target = gen.clone()
        # target[target==0] = -100
        # loss = model(**inputs, labels=target.unsqueeze(0))['loss'].item()
        assert(torch.isnan(ppl) == False and ppl < 1e4)
        outputs.append(gen_text)
        ppls.append(ppl.item())
    return outputs, ppls

def generate_opt(url, prompt, model, processor, gen_mode='C',
             penalty_alpha=0.6, top_k=4, temperature=1, do_sample=True, 
             top_p=0.95, min_length=0, n_samples=10, max_length=100, half_prec=True):
    outputs = []
    ppls = []
    ...
    for idx in range(generation['sequences'].shape[0]):
        gen = generation['sequences'][idx]
        gen_text = processor.decode(gen, skip_special_tokens=True)
        valid_gen_idx = torch.where(gen!=1)[0][1:]
        logits = torch.vstack([generation['scores'][i][idx].unsqueeze(0) for i in valid_gen_idx-1])
        ppl = compute_perplexity(logits, gen[gen!=1][1:])
        assert(torch.isnan(ppl) == False and ppl < 1e4)
        outputs.append(gen_text)
        ppls.append(ppl.item())
...
if 'blip' in args.model:
    ...
    if args.half_precision:
        ...
        model = Blip2ForConditionalGeneration.from_pretrained(args.model, device_map="auto", torch_dtype=torch.float32)
...
with torch.no_grad():
    for split in ['train', 'val', 'test']:
        for i in tqdm(range(len(pid_splits[split]))):
            ...
            if 'blip' in args.model and 't5' in args.model:
                gen_desc, ppl = generate(url, prompt, model, processor, gen_mode=args.gen_mode,
                                    ...
                                    half_prec=args.half_precision)
            elif 'blip' in args.model and 'opt' in args.model:
                gen_desc, ppl = generate_opt(url, prompt, model, processor, gen_mode=args.gen_mode,
                                    penalty_alpha=args.penalty_alpha, top_k=args.top_k, 
                                    temperature=args.temperature, do_sample=args.do_sample, 
                                    top_p=args.top_p, min_length=args.min_length, 
                                    n_samples=args.n_samples, max_length=args.max_length, 
